...
const OKStatusResponsesComponents={
  "google": (
    <Alert severity="success">
        Votre inscription a réussi ! Vous pouvez désormais accéder à votre compte.
    </Alert>),
  "credentials": (<Alert severity="info">
  Pour continuer votre inscription, veuillez ouvrir le lien envoyé à votre adresse e-mail.
 </Alert>)
}
...
function SignUpForm() {
    ...
    return (
      ...
      <Modal open={isOpen} onClose={onClose} disableAutoFocus>
            ...
            <h1 className='text-center text-2xl'>Vérifiez votre boîte de réception.
</h1>
            <p className='text-center'>
            Cliquez sur le lien que nous avons envoyé à {auth.currentUser?.email} pour compléter la configuration de votre compte.
            </p>
      <button onClick={onClose} className="text-white bg-primary-color py-2 px-6 font-bold rounded-full">OK</button>
        ...
        {isRegistering&&<AppSpinner size={50} variant="LIGHT" className="fixed top-0 right-0 m-4"/>}
            <h1 className="text-primary-color text-4xl font-bold pb-11 text-center">S'inscrire</h1>
            <form onSubmit={onSubmit} className="flex flex-col gap-4">
            {response.status=="ERROR"?<Alert severity="error">Une erreur est survenue</Alert>:response.status=="OK"?
...
<FormControl error={!!errors?.error.acceptPlcs}>
    {errors?.error.acceptPlcs&&<span className="text-[0.75rem] text-[#d32f2f]">{errors?.error.acceptPlcs}</span>}
                <FormControlLabel sx={{
                  ...
                  checked={initialUser.acceptPlcs} onChange={()=>setInitialUser(prev=>({...prev,acceptPlcs:!prev.acceptPlcs}))}/>} label="Je suis d'accord avec la politique de confidentialité et les conditions d'utilisation de ce site web" />
                  </FormControl>
                <BasicButton type="submit">S'inscrire</BasicButton>
            </form>
            <div className="flex flex-col pt-4">
            <BrandButton onClick={async ()=>{
              const response=await doSignInWithGoogle()
              setResponse(response)
              }} url="/google.png">S'inscrire avec Google</BrandButton>
                <span className="text-center font-bold mt-2">Vous avez déjà un compte? <Link href={"/login"} className="underline text-primary-blue">Se connecter</Link></span>
                </div>
        </div>
        </> 
    );
