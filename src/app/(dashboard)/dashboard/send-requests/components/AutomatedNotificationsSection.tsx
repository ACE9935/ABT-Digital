  ...
  async function deleteAutomatedNotification(userId: string, notificationId: string) {
    try {
      ...
      toast({
        ...
        render: () => (
          <AppToast variant="SUCCESS" title="Notification supprimée" Icon={Check} />
        ),
      });
    }
  }
  ...
  async function toggleAutomatedNotification(userId: string, notificationId: string, activate: boolean) {
    try {
      ...
      toast({
        position: 'bottom-left',
        render: () => (
          <AppToast variant="SUCCESS" title={`Notification ${activate ? 'activée' : 'désactivée'}`} Icon={Check} />
        ),
      });
    }
  }
    ...
    <>
      <Modal open={isOpen} onClose={onClose} disableAutoFocus>
        <div
          ...
          <p className="text-xl">Voulez-vous vraiment supprimer cette notification ?</p>
          <div className="flex gap-3">
            ...
            <button onClick={async () => await deleteAutomatedNotification(userId, notification.id)} className="bg-primary-color font-bold text-white rounded-full p-3 flex gap-2 items-center">{loading && <AppSpinner variant="LIGHT" size={26} />}Supprimer</button>
          </div>
        </div>
      </Modal>
      <Modal open={isOpenY} onClose={onCloseY} disableAutoFocus>
        <div
          ...
          <div className="flex gap-3">
            <button className="border-2 border-primary-color font-bold text-black rounded-full p-3 flex gap-2 items-center" onClick={onCloseY}>Annuler</button>
            <button onClick={async () => await toggleAutomatedNotification(userId, notification.id, !notification.activated)} className="bg-primary-color font-bold text-white rounded-full p-3 flex gap-2 items-center">{loading && <AppSpinner variant="LIGHT" size={26} />}{notification.activated ? 'Désactiver' : 'Activer'}</button>
          </div>
        </div>
      </Modal>
      <div className={`rounded-md bg-white shadow-lg p-3 px-5 flex flex-col gap-4 transition-height ${expand ? "h-auto" : "h-[64px]"} overflow-hidden`}>
        <div className="flex justify-between items-center w-full font-bold">
          ...
          <div className="flex gap-4 items-center">
            <div className="flex gap-2">
              Statut: {notification.activated ?
                <div className="flex items-center">
                  <div className="rounded-full bg-green-400 w-4 aspect-square"></div>
                  <div className="font-normal text-slate-600 pl-3">Actif</div>
                </div> :
                <div className="flex items-center">
                  <div className="rounded-full bg-red-500 w-4 aspect-square"></div>
                  <div className="font-normal text-slate-600 pl-3">Inactif</div>
                </div>}
            </div>
        ...
        <div className="border-t border-slate-600 py-5">
          <div className="text-slate-600 flex gap-4 font-semibold">
            <div className="border-r border-slate-500 pr-4">Prochain envoi: {notification.scheduledDate}</div>
            <div>Fréquence: chaque {getFrequencyInFrench(notification.every)}</div>
          </div>
          <div className="mt-4">
            <div>Sujet:<br />
          ...
          <div className="mt-5 flex justify-end gap-3">
            <button onClick={() => {
              setActiveStatusText(`Voulez-vous vraiment ${notification.activated ? 'désactiver' : 'activer'} cette notification ?`);
              onOpenY();
            }} className="bg-primary-color rounded-full p-3 text-white font-bold">{notification.activated ? 'Désactiver' : 'Activer'}</button>
            <button onClick={onOpen} className="bg-red-600 rounded-full p-3 text-white font-bold">Supprimer</button>
          </div>
        </div>
      </div>
    </>
...
function AutomatedNotificationsSection() {
  ...
  return (
    <div>
      <h2 className="pb-4 font-bold text-xl">Emails automatisés</h2>
