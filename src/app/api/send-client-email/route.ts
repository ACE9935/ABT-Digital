...
export async function POST(req: Request): Promise<Response> {
  try {
    const { emails, emailContent, sender, subject, automate, automateValue, automateType, userId } = await req.json();

    // Send the email
    await sendClientEmail({ emailContent, subject, identifier: emails,sender });
