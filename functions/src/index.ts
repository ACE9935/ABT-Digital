...
exports.resetScansPerDay = onSchedule('every day 00:00', async () => {
  try {
    ...
    // Calculate yesterday's date for scansForDate
    ...
    snapshot.forEach(async (doc) => {
      ...
      data.automatedNotifications.forEach(async (notification: AutomatedNotification) => {
        if (notification.scheduledDate === today.toISOString().split('T')[0]) {
          ...
          if (notification.type === "Email") {
            ...
            const emails = data.clientEmails as string[];
            await sendClientEmail({ emailContent, sender, subject, identifier: emails })// Ensure promise resolves with 'undefined'
          } 
